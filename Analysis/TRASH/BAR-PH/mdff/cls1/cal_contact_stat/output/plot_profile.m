...
str_prefix=['cls1-gs3-water-contact-I1-'];
figure(1);clf;
cc=hsv(4);
kk=0;
p=0;
...
str_print=['PLOT_cls1-gs3-water_contact_full_I1'];
print(figure(1),'-r500','-dpng',str_print);

str_prefix=['cls1-gs3-water-contact-I2-'];
figure(2);clf;
cc=hsv(4);
kk=0;
p=0;
for ii=2:5
    ...
    str_legend{kk}=['pair#',num2str(ii-1)];
end
legend(p,str_legend,'fontsize',12,'location','northeastoutside');
xlabel('Time (ns)','FontSize',20);
ylabel('# of contacts','FontSize',20);
...
str_print=['PLOT_cls1-gs3-water_contact_full_I2'];
