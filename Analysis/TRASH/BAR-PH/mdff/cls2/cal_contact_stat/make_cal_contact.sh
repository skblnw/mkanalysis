#!/bin/bash
FILENAME=output/cls2-gs3-water
...
if true; then
    ...
    sed -e 's/NUMLIST/5 6 1 2 3 4 7 8/g' template-cal-contact > cal_contact.tcl
    while read line; do
        vmd -dispdev text -e cal_contact.tcl -args $FILENAME $line </dev/null
        wait
    done < TMP
    ...
    sed -e 's/NUMLIST/5 6 1 2 3 4 7 8/g' template-cal-contact-all > cal_contact-all.tcl
