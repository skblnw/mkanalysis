...
id_outfile = fopen('cls2.txt','w');
fprintf(id_outfile,'%3.1f\t',data_plot(3,264:end));
...
set(bb(2),'facecolor','b');
set(bb(2),'edgecolor','b');
set(bb(3),'facecolor','r');
set(bb(3),'edgecolor','r');

xlim([0 361]);
set(gca,'XTick',1:20:361);
xlabel('Residue ID','FontSize',20);
ylabel('# of contacts','FontSize',20);
legend({'Region I1','Region I2 (underneath)','Region I2 (above)'},'FontSize',16,'Location','northwest');
% legend('boxoff');
title('# of contacts at residues','FontSize',24);
grid on;

str_print=['PLOT_cls2-4dimer-full-resseq-all'];
% print(figure(1),'-r500','-dpng',str_print);


figure(2);clf;
par_range={[1 25],[26 97],[98 115],[116 140],[141 147],[148 180],[181 227],[228 249],[250 270],[271 287],[288 300],[306 322],[323 342],[343 361]};
for nn=1:length(par_range)
    ...
    xlim(par_range{nn});
    if ismember(nn,[1 3 4 6 8 9 10 12 13])
        ylim([0 15]);
        set(gca,'YTick',[5 10]);
        set(gca,'fontsize',6);
...
str_print=['PLOT_cls2-4dimer-full-resseq-detail'];
