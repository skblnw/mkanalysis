...
proc countHeavy { nn sel_input } {
    ...
    foreach ii [lsort -unique -integer [\$sel get residue]] {
        set tmplist 0
        foreach dd \$dlist {
            set selheavy [atomselect top "name CA and not residue \$ii and same residue as noh and not backbone and within \$dd of residue \$ii" frame \$nn]
...
for {set nn 0} {\$nn < \$total_frame} {incr nn} {
    ...
    foreach {sel_input} {"segname MUT"} {
        ...
        # Write to file
        puts \$outf [countHeavy \$nn \$sel_input]
