...
proc vmd_draw_principalaxes { mol sel ii {weights domass}} {
    if { $weights == "domass" } {
    ...
    set scale [expr .5*[Orient::max [lindex $ranges 0] \
                             [lindex $ranges 1] \
                             [lindex $ranges 2]]]
    set scale2 [expr 1.02 * $scale]

    set filename [join "draw$ii.tcl"]
    source $filename
    ...
    # graphics $mol text [vecadd $COM [vecscale $scale2 $a1]] "1" size 3 thickness 4
