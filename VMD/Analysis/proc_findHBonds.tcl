# Find hydrogen bonds within "dist" and angle "ang"
# Add atom indexes to selections to make the script compute results for atom pairs
proc findHBonds { nn seg sel_input1 sel_input2 dist ang } {
    set sel1 [atomselect top "(protein and {oxygen or nitrogen or sulfur} and not backbone) and segname D$seg and resid $sel_input1" frame $nn]
    set sel2 [atomselect top "(protein and {oxygen or nitrogen or sulfur} and not backbone) and not segname D$seg and resid $sel_input2" frame $nn]
    set tmpList [measure hbonds $dist $ang $sel1 $sel2]
    $sel1 delete
    $sel2 delete
    ...
    set sel1 [atomselect top "(protein and {oxygen or nitrogen or sulfur} and not backbone) and segname D$seg and resid $sel_input2" frame $nn]
    set sel2 [atomselect top "(protein and {oxygen or nitrogen or sulfur} and not backbone) and not segname D$seg and resid $sel_input1" frame $nn]
    set tmpList [measure hbonds $dist $ang $sel1 $sel2]
    $sel1 delete
    $sel2 delete
